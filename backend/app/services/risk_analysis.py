...
def calculate_risk_scores(db: Session, scenario_context_id: int) -> Dict[str, Any]:
    scenario = db.query(RiskScenario).filter(RiskScenario.id == scenario_context_id).first()
    if not scenario:
        return {
            "initial_score": 0,
            "residual_score": 0,
    ...
    impact_ratings = db.query(RiskContextImpactRating).filter(
        RiskContextImpactRating.risk_scenario_context_id == scenario_context_id
    ).all()
    ...
    control_effects = db.query(ControlEffectRating).filter(
        ControlEffectRating.risk_scenario_id == scenario_context_id
    ).all()
    effect_map = {}
