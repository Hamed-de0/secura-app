...
export default function ThreatModal({ open, onClose, onSelect, linkedIds = new Set() }) {
  const [items, setItems] = useState([]);
  const [category, setCategory] = useState('All');
  const [hideLinked, setHideLinked] = useState(true);

  useEffect(() => { if (open) getThreats().then(setItems).catch(() => setItems([])); }, [open]);
  ...
  const columns = [
    ...
    {
      field: 'actions', headerName: 'Action', width: 140, renderCell: (params) => {
        const linked = linkedIds.has(params.row.id);
        return linked
    ...
    <Modal open={open} onClose={onClose}>
      <Box sx={{ p: 3, bgcolor: 'background.paper', m: '6% auto', width: '86%', borderRadius: 2, boxShadow: 24 }}>
        <Stack direction="row" spacing={2} alignItems="center" mb={2}>
          <Typography variant="h6" sx={{ flexGrow: 1 }}>Select Threat</Typography>
