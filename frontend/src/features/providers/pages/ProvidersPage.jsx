...
export default function ProvidersPage() {
  ...
  const gridView = useGridView({ key: 'providers/list@v1', defaults: { pagination:{ pageSize: 10 }, columns:{ visible:[], order:[] }, sort:[], density:'standard', filters:{ q:'' } }, filterSchema:{ q:'' } });
  ...
  const filtered = useMemo(() => {
    ...
    return (items || []).filter(s => _q ? (`${s.name ?? ''} ${s.description ?? ''}`.toLowerCase().includes(_q)) : true);
  }, [items, q]);

  if (isLoading) return <Skeleton variant="rounded" height={360} />;
  if (isError) return <ErrorState icon={CloudSyncIcon} title="Failed to load" description={error?.message || 'Error'} />;
  ...
  return (
    <Box>
      ...
      <Stack direction="row" spacing={2} sx={{ mb: 1 }}>
        <TextField size="small" placeholder="Search providers…" value={q} onChange={(e)=> gridView.setFilters({ ...gridView.snapshot.filters, q: e.target.value })} sx={{ minWidth: 260 }} />
      </Stack>
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={4}>
          ...
          {!isLoading && filtered?.length === 0 && (
            <EmptyState title="No services" description="No services to show." />
          )}
          {!isLoading && filtered?.length > 0 && (
            <ProviderList items={filtered} selectedId={selectedId} onSelect={setSelectedId} />
          )}
        </Grid>
        <Grid item xs={12} md={8}>
          {isLoading && <Skeleton variant="rounded" height="100%" />}
          {!isLoading && filtered?.length === 0 && (
            <EmptyState title="Select a service" description="No services to show at this scope." />
          )}
          {!isLoading && filtered?.length > 0 && <ProviderDetail service={svc} />}
        </Grid>
      </Grid>
    </Box>
  );
