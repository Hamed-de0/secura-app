...
export default function ControlsAtScope() {
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter(c => {
      ...
      if (assurance && (c.assurance_status || '').toLowerCase() !== assurance) return false;
      if (q) {
        const needle = q.toLowerCase();
        const hay = `${c.code ?? ''} ${c.title ?? ''} ${c.notes ?? ''}`.toLowerCase();
  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      <Stack direction="row" spacing={1} sx={{ mb: 1 }} flexWrap="wrap">
        {Object.entries(quickCounts.bySource).map(([k, v]) => <Chip key={k} size="small" label={`${k}: ${v}`} />)}
        {Object.entries(quickCounts.byAssurance).map(([k, v]) => <Chip key={k} size="small" label={`${k}: ${v}`} />)}
      </Stack>
